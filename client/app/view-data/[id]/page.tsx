...
export default function ViewData({ params }: {params: {id: string}}) {
    ...
    const fetchData = async () => {
        const req = fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/${entityType.toLowerCase()}s/${inputValue}`);
        const res = await req;
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-between p-24">
        <div className="flex items-center justify-center mx-2">
            ...
            <input type="text" name="id" id="id" className="mx-2" onInput={handleInput}/>
        </div>
